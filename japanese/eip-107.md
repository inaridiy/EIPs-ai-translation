---
original: bbffb679932dadbd4d79b29f8488ccefb96f6277b7e8a2a406ef17045a8dd367
---
...
このEIPドラフトは、RPC対応のイーサリアムノードが提供する認証方法の詳細について説明しています。この方法を使えば、CORSを有効にしなくても、一般のウェブサイトからトランザクション(```eth_sendTransaction```)を送信できるようになります。その代わりに、ユーザーにHTMLポップアップを通じてトランザクションの確認を求めます。

DAppが実行したい読み取り専用のRPCコールはノードのドメインの不可視のiframeにリダイレクトされ、DAppが実行したいトランザクションについては、ユーザーにキャンセルまたは確認を求めるHTMLポップアップが表示されます。これにより、DAppはノードのRPCAPIに接続できますが、特別な権限は与えられません。ユーザーのアカウントがロック解除された状態で、日常的なウェブブラウザ上でDAppと安全にやり取りできるようになります。アカウントがロックされていて、ノードが"personal"APIをRPCで公開している場合は、HTMLページでユーザーにパスワードを入力してアカウントをロック解除することもできます。

## 動機

現在、ユーザーが日常的に使用しているブラウザでDAppを実行する場合、セキュリティ上の理由から、DAppはデフォルトでRPCAPIにアクセスできません。ユーザーはDAppが動作するためにウェブサイトのドメインでCORSを有効にする必要があります。しかし、CORSを有効にすると、ユーザーの同意なしにDAppが任意のロック解除アカウントからトランザクションを送信できるようになります。つまり、ユーザーはノードの既定の設定を変更する必要があり、さらにDAppを信頼しなければならないのです。これは受け入れられるものではなく、既存のDAppは次のようなワークアウトに頼らざるを得ない状況です:
- イーサ送金の場合は、ユーザーに"Mist"などの専用の信頼できるウォレットでトランザクションを入力させる
- より複雑な場合は、ユーザーにノードのコマンドラインインターフェイスでトランザクションを手動で入力させる

この提案は、安全でユーザーフレンドリーな代替手段を提供することを目的としています。

以下は、提案する実装のHTMLポップアップのスクリーンショットです:

### アカウントがロック解除されている場合

アカウントがすでにロック解除されている場合、DAppが行おうとするすべてのトランザクションについて、次のようなポップアップが表示されます:

![](../assets/eip-107/authorization.png)

### アカウントがロックされており、"personal"APIがRPCで公開されていない場合:

アカウントがロックされていて、ノードがRPCインターフェイスでアカウントのロック解除を許可していない場合、次のようなポップアップが表示されます。これは理想的ではありませんが、ユーザーがアカウントのロック解除方法を知っている必要があります:

![](../assets/eip-107/authorization-locked.png)

### アカウントがロックされているが、ノードが"personal"APIをRPCで公開している場合:

より良い選択肢は、ユーザーにパスワードを求めることです。ただし、これはノードが"personal"APIをRPCで公開している場合にのみ可能です。その場合、ユーザーがアカウントのロック解除に必要なパスワードを入力して承認できるようなダイアログが表示されます:

![](../assets/eip-107/authorization-password.png)

## 仕様

この仕組みを機能させるには、ノードが\<ノードURL\>/authorization.htmlのURLでHTMLファイルを提供する必要があります。

このファイルは、DAppによって2つの異なるモード(不可視のiframeとポップアップウィンドウ)で使用されます。

不可視のiframeはDAppに埋め込まれ、DAppがCORSを有効にしなくてもreadonly RPCコールを送信できるようにします。これは、iframeとノードが同じドメイン/ポートを共有しているため、JavaScriptの```window.postMessage```を使ってメッセージを送信することで実現しています。

iframeモードでは、HTMLファイルのJavaScriptコードが、ロック解除されたキーを必要とするコールを行えないようにします。これは、DAppがiframeを埋め込んでユーザーに確認ボタンをクリックさせるのを防ぐためです。
DAppが```eth_sendTransaction```コールを必要とする場合、DAppは代わりに同じURLを使ってポップアップウィンドウを開きます。

このポップアップウィンドウモードでは、HTMLファイルのJavaScriptコードが```eth_sendTransaction```(ただし```eth_sign```は、トランザクションの内容を安全に表示する方法がないため)を許可します。ただし、ノードに直接送信するのではなく、送信元アドレス、受信先アドレス、送金額、ガスコストなどを表示する確認ダイアログが表示されます。ユーザーが承認すると、リクエストが送信され、結果がDAppに返されます。ユーザーがキャンセルした場合はエラーが返されます。

HTMLページでは"personal"APIの有無も確認し、必要な場合はユーザーにアカウントのロック解除を求めます。ロック解除は一時的(3秒)なので、この短い時間内にトランザクションを試みると、再度パスワードの入力を求められます。

iframeモードとウィンドウモードの両方で、DAppとの通信は```window.postMessage```を使って行われます。
iframeやウィンドウが最初に送信するメッセージは"ready"という文字列で、DAppにメッセージを受け付ける準備ができたことを知らせます。その後、DAppはメッセージを送信できるようになります。メッセージは次のようなオブジェクトの形式で送信されます:
```
{
  id:<id>, //レスポンスを照合するためのID。順序は保証されない
  payload:<JSON-RPCオブジェクト> //通常ノードに送信されるのと同じオブジェクト
}
```

```eth_sendTransaction```の場合、"gas"、"gasPrice"、"from"フィールドをRPCパラメーターに設定する必要があります。これらのいずれかが欠けている場合、ウィンドウは適切な値を表示できません。

メッセージを受け取ると、iframeは実際のRPCコールをノードに行いますが、ロック解除されたキーを必要としないコールのみ許可されます。それ以外の場合はエラーを返します。ウィンドウの場合は```eth_sendTransaction```コールのみ受け付けますが、ユーザーの承認を求めるダイアログを表示します。

すべての場合において、iframeやウィンドウはDAppに次のようなオブジェクトでメッセージを送り返します:
```
{
  id:<リクエストのID>,
  result:<RPCの結果>,
...
エラーオブジェクトは、postMessageの制限により、JavaScriptの```Error```オブジェクトではなく、次のような形式になります:
```
{
  message:<エラーの説明>,
  type:<エラーの種類> //typeが"cancel"の場合はユーザーがトランザクションをキャンセルしたことを示す
}
```

## 根拠

この提案の設計は、シンプルさとセキュリティを重視して選択されました。以前の案では、ユーザーがトランザクションリクエストを承認または拒否するためのOAuth風のプロトコルを使うことを検討していました。しかし、それにはノードのコードに大きな変更が必要で、一部のgethコントリビューターからは、DAppを意識したコードはgethのコードベースに適切ではないと指摘されていました。
現在の設計では、ノードの実装間で共有できる自己完結型のHTMLファイルという非常にシンプルな実装になっており、ブラウザのクロスドメインポリシーによってセキュリティが保証されています。

iframeとウィンドウを使う理由は、セキュリティとユーザビリティのバランスを取るためです。不可視のiframeにより、ユーザーの入力なしでDAppが読み取り専用のコールを実行できるようになり、ウィンドウにより、ユーザーの承認を得てからコールを実行できるようになります。iframeの確認ダイアログをネイティブの```window.confirm```ダイアログにすることも考えましたが、より洗練された確認ポップアップを使えなくなるため、この設計を採用しました。また、```window.confirm```は一部のブラウザで安全ではないことが知られています(https://bugs.chromium.org/p/chromium/issues/detail?id=260653)。

## 実装

この設計を実装するには、\<ノードURL\>/authorization.htmlにこのHTMLファイルまたは同等のものを提供するだけです。
...
,h-=r),o>0&&h>0){for(r=h%o||o,f=a.substr(0,r);h>r;r+=o)f+=s+a.substr(r,o);u>0&&(f+=s+a.slice(r)),c&&(f="-"+f)}t=l?f+X.decimalSeparator+((u=+X.fractionGroupSize)?l.replace(new RegExp("\\d{"+u+"}\\B","g"),"{{MARKDOWN_DOCUMENT}}"+X.fractionGroupSeparator):l):f}return t},T.toFraction=function(e){var n,t,i,o,u,s,f,l,c,a=j,h=this,g=h.c,p=new E(q),d=t=new E(q),m=f=new E(q);if(null!=e&&(j=!1,s=new E(e),j=a,(!(a=s.isInt())||s.lt(q))&&(j&&L(22,"max denominator "+(a?"out of range":"not an integer"),e),e=!a&&s.c&&U(s,s.e+1,1).gte(q)?s:null)),!g)return h.toString();for(c=r(g),o=p.e=c.length-h.e-1,p.c[0]=S[(u=o%O)<0?O+u:u],e=!e||s.cmp(p)>0?o>0?p:d:s,u=z,z=1/0,s=new E(c),f.c[0]=0;l=C(s,p,0,1),i=t.plus(l.times(m)),1!=i.cmp(e);)t=m,m=i,d=f.plus(l.times(i=d)),f=i,p=s.minus(l.times(i=p)),s=i;return i=C(e.minus(t),m,0,1),f=f.plus(i.times(d)),t=t.plus(i.times(m)),f.s=d.s=h.s,o*=2,n=C(d,m,o,k).minus(h).abs().cmp(C(f,t,o,k).minus(h).abs())<1?[d.toString(),m.toString()]:[f.toString(),t.toString()],z=u,n},T.toNumber=function(){return+this},T.toPower=T.pow=function(e,n){var t,r,i,o=d(0>e?-e:+e),u=this;if(null!=n&&(M=23,n=new E(n)),!H(e,-y,y,23,"exponent")&&(!isFinite(e)||o>y&&(e/=0)||parseFloat(e)!=e&&!(e=NaN))||0==e)return t=Math.pow(+u,e),new E(n?t%n:t);for(n?e>1&&u.gt(q)&&u.isInt()&&n.gt(q)&&n.isInt()?u=u.mod(n):(i=n,n=null):J&&(t=p(J/O+2)),r=new E(q);;){if(o%2){if(r=r.times(u),!r.c)break;t?r.c.length>t&&(r.c.length=t):n&&(r=r.mod(n))}if(o=d(o/2),!o)break;u=u.times(u),t?u.c&&u.c.length>t&&(u.c.length=t):n&&(u=u.mod(n))}return n?r:(0>e&&(r=q.div(r)),i?r.mod(i):t?U(r,J,k):r)},T.toPrecision=function(e,n){return F(this,null!=e&&H(e,1,A,24,"precision")?0|e:null,n,24)},T.toString=function(e){var n,t=this,i=t.s,o=t.e;return null===o?i?(n="Infinity",0>i&&(n="-"+n)):n="NaN":(n=r(t.c),n=null!=e&&H(e,2,64,25,"base")?D(l(n,o),0|e,10,i):B>=o||o>=$?f(n,o):l(n,o),0>i&&t.c[0]&&(n="-"+n)),n},T.truncated=T.trunc=function(){return U(new E(this),this.e+1,1)},T.valueOf=T.toJSON=function(){var e,n=this,t=n.e;return null===t?n.toString():(e=r(n.c),e=B>=t||t>=$?f(e,t):l(e,t),n.s<0?"-"+e:e)},null!=e&&E.config(e),E}function t(e){var n=0|e;return e>0||e===n?n:n-1}function r(e){for(var n,t,r=1,i=e.length,o=e[0]+"";i>r;){for(n=e[r++]+"",t=O-n.length;t--;n="0"+n);o+=n}for(i=o.length;48===o.charCodeAt(--i););return o.slice(0,i+1||1)}function i(e,n){var t,r,i=e.c,o=n.c,u=e.s,s=n.s,f=e.e,l=n.e;if(!u||!s)return null;if(t=i&&!i[0],r=o&&!o[0],t||r)return t?r?0:-s:u;if(u!=s)return u;if(t=0>u,r=f==l,!i||!o)return r?0:!i^t?1:-1;if(!r)return f>l^t?1:-1;for(s=(f=i.length)<(l=o.length)?f:l,u=0;s>u;u++)if(i[u]!=o[u])return i[u]>o[u]^t?1:-1;return f==l?0:f>l^t?1:-1}function o(e,n,t){return(e=c(e))>=n&&t>=e}function u(e){return"[object Array]"==Object.prototype.toString.call(e)}function s(e,n,t){for(var r,i,o=[0],u=0,s=e.length;s>u;){for(i=o.length;i--;o[i]*=n);for(o[r=0]+=N.indexOf(e.charAt(u++));r<o.length;r++)o[r]>t-1&&(null==o[r+1]&&(o[r+1]=0),o[r+1]+=o[r]/t|0,o[r]%=t)}return o.reverse()}function f(e,n){return(e.length>1?e.charAt(0)+"."+e.slice(1):e)+(0>n?"e":"e+")+n}function l(e,n){var t,r;if(0>n){for(r="0.";++n;r+="0");e=r+e}else if(t=e.length,++n>t){for(r="0",n-=t;--n;r+="0");e+=r}else t>n&&(e=e.slice(0,n)+"."+e.slice(n));return e}function c(e){return e=parseFloat(e),0>e?p(e):d(e)}var a,h,g=/^-?(\d+(\.\d*)?|\.\d+)(e[+-]?\d+)?$/i,p=Math.ceil,d=Math.floor,m=" not a boolean or binary digit",w="rounding mode",v="number type has more than 15 significant digits",N="0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ$_",b=1e14,O=14,y=9007199254740991,S=[1,10,100,1e3,1e4,1e5,1e6,1e7,1e8,1e9,1e10,1e11,1e12,1e13],R=1e7,A=1e9;if("undefined"!=typeof crypto&&(a=crypto),"function"==typeof define&&define.amd)define(function(){return n()});else if("undefined"!=typeof module&&module.exports){if(module.exports=n(),!a)try{a=require("crypto")}catch(E){}}else e||(e="undefined"!=typeof self?self:Function("return this")()),e.BigNumber=n()}(this);
  ...
  <body>
    ...
    <div id="pleasewait">
      <br/>
      <p id="infomessage">しばらくお待ちください...</p>
    </div>

    <form id="form" class="wrapper">
      <br/>
      <p id="message" class="message"></p>
      <p id="passwordField"><label>パスワードが必要です:</label><input id="password" type="password" /></p>
      <button id="cancel-button" type="button" autofocus>キャンセル</button>
      <button id="confirm-button" type="button" >確認</button>
    </form>

    <div id="modal-dialog" class="wrapper">
      <h3 id="modal-dialog-title" class="title">タイトル</h3>
      <p id="modal-dialog-message" class="message">メッセージ</p>
...
.onreadystatechange = function() {
      ...
      try {
        ...
        callback({message:'接続エラー: ノード ' + url + ' に接続できませんでした。',type:"noConnection"});
      }
    }
    ...
    function askAuthorization(transactionInfo, data, requireUnlock, sourceWindow){
      ...
      cancelButton.onclick = function(){
        sourceWindow.postMessage({id:data.id,result:null,error:{message:"承認されませんでした"},type:"cancel"},firstUrl);
        closeWindow();
      }
      ...
      var submitFunc = function(){
        window.onbeforeunload = function (event) {
          if(!closedByCode){
            return "トランザクションの処理中です。キャンセルはできず、結果を待つ必要があります。";
          }
        };
        if(requireUnlock){
          if(password.value == ""){
    ...
    function receiveMessage(event){
      ...
      try{
        ...
        event.source.postMessage({id:data.id,result:null,error:{message:"メッセージデータを処理できませんでした"},type:"notValid"},firstUrl);
        showMessage("エラー","アプリケーションが無効なデータを送信しました", function(){
          closeWindow();
        });
      }
    ...
    function processMessage(data, sourceWindow){
      ...
      if(inIframe){
        if(isMethodAllowed(data.payload.method)){
          ...
          sourceWindow.postMessage({id:data.id,result:null,error:{message:"メソッド (" + data.payload.method + ") はiframeでは許可されていません"},type:"notAllowed"},firstUrl);
        }
        ...
        if(transactionInfo != null){
          needToAndCanUnlockAccount(transactionInfo.from,data.url,function(requireUnlock,canUnlock){
            if(requireUnlock && canUnlock){
              ...
              var span = document.createElement('span');
              span.innerHTML = "アカウントのロックを解除する必要があります: <br/>" + transactionInfo.from;
              messageHtml.appendChild(span);

              showMessage("アカウントがロックされています",messageHtml,function(){
                processMessage(data,sourceWindow);
              }, "完了");
            }
          ...
          sourceWindow.postMessage({id:data.id,result:null,error:{message:"from、to、gas、gasPriceを指定する必要があります"},type:"notValid"},firstUrl);
          closeWindow();
        }
      }else{
        sourceWindow.postMessage({id:data.id,result:null,error:{message:"メソッド (" + data.payload.method + ") はポップアップでは許可されていません"},type:"notAllowed"},firstUrl);
      }     
    }

    function checkMessageNotReceived(){
      if(noMessageReceivedYet){
        showMessage("エラー","トランザクションを受信できませんでした。ポップアップがブロックされていないことを確認してください。", function(){
          closeWindow();
        });
      }
    }
